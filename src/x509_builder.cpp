    ...
    namespace builder {
        ...
        namespace _ {
            ...
            owned<STACK_OF(GENERAL_NAME)> make_stack(IterT const& begin, IterT const& end)
            {
                ...
                std::for_each(begin, end, [&names](auto const& name) -> void { names.push(general_name::copy(name)); });
            ...
            std::function<void(context&, ossl::x509_req::roref&)> func)
        {
            ...
            if (!evp_pkey::equal(pubkey, pubkey2))
                CPPOSSL_THROW_ERRNO(EINVAL, "X.509 certificate public key was set but it does not match signing key.");
        ...
        void copy_extensions::some(context& ctx, ossl::x509_req::roref& req, std::initializer_list<object::nid> nids)
        {
        ...
        {
            owned<::ASN1_INTEGER> i = asn1::integer::make(serial);
            if (X509_set_serialNumber(ctx.get(), i.get()) == 0)
                CPPOSSL_THROW_LAST_OPENSSL_ERROR("Failed to set X.509 serial number."); // LCOV_EXCL_LINE
        }
        ...
        {
            owned<::BIGNUM> serial = bignum::random();
            return set_serialno(ctx, ossl::raii::roref<::BIGNUM>(serial));
        }
